...
const UserProfile = ({ backgroundImage, className, border, divFunction }) => {
  ...
  useEffect(() => {
    setLinksArray(links.concat(Array(5 - links.length).fill({ secondary_icon: link_black_icon, platform_name: "Empty", platform_link: "", empty: true })));
  }, [links]);
  ...
  return (
    <div {...(backgroundImage && { style: { backgroundImage: `url(${phone_placeholder_icon})` }, })} className={`relative bg-contain bg-no-repeat bg-center flex flex-col gap-y-3 md:gap-y-10 lg:gap-y-8 items-center justify-center ${className}`}>
      <div style={{ ...(details.profile && { backgroundImage: `url(${details.profile})` }) }} className={`w-20 h-20 md:w-32 md:h-32 lg:h-24 lg:w-24 ${details.profile ? "bg-cover bg-no-repeat border border-purple-custom" : "bg-grey-deep"} ${border ? "border-l-purple-custom" : ""} rounded-full`}></div>
      <div className={`flex flex-col items-center ${(details.firstname || details.lastname || details.email) ? "" : "gap-y-2"} text-center w-full`}>
        <p className={`${(details.firstname || details.lastname) ? "text-xs md:text-2xl lg:text-base w-full" : "bg-grey-deep w-32 h-2.5 rounded-3xl"}`}>{(details.firstname || details.lastname) ? details.firstname + " " + details.lastname : ""}</p>
