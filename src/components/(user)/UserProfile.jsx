  ...
  return (
    <div {...(backgroundImage && { style: { backgroundImage: `url(${phone_placeholder_icon})` }, })} className={`relative bg-contain bg-no-repeat flex flex-col gap-y-3 md:gap-y-10 lg:gap-y-8 items-center justify-center ${className}`}>
      ...
      <div className={`flex flex-col items-center ${(profileDetails.first_name || profileDetails.last_name || profileDetails.email) ? "" : "gap-y-2"} text-center w-full`}>
        <p className={`${(profileDetails.first_name || profileDetails.last_name) ? "text-xs md:text-2xl lg:text-base w-full" : "bg-grey-deep w-32 h-2.5 rounded-3xl"}`}>{profileDetails.first_name + " " + profileDetails.last_name}</p>
        <p className={`${profileDetails.email ? "text-[8px] md:text-base lg:text-xs w-full break-all" : "bg-grey-deep w-20 h-1.5 rounded-3xl"}`}>{profileDetails.email}</p>
      </div>
