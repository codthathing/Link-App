...
const LinkTemplate = ({ id, name, iconValue, linkValue, platformValue, handleInput, placeholder, errorMessage, setErrorMessage, removeFunction, link = true, readOnly = true, showIcon = false, showOption, setShowOption, availableLinks, changeOption }) => {
  return (
    <div key={id} className="bg-grey-light rounded-md p-4 md:p-6">
      ...
      <div className="flex flex-col gap-y-2 md:gap-y-4 lg:gap-y-3">
        <main className="relative">
          ...
          <div onClick={() => setShowOption(!showOption)} className={`flex items-center py-1 px-3 md:px-5 gap-x-2 md:gap-x-4  border border-grey-semilight rounded-md ${showIcon ? "cursor-pointer" : ""}`}>
            <img src={iconValue} className="w-3 md:w-5 lg:w-4" alt="GITHUB ICON" />
            <input type="text" value={platformValue} onChange={handleInput} className={`text-[8px] md:text-lg outline-none shadow-inset shadow-grey-light w-full py-2 md:py-3 ${showIcon ? "cursor-pointer" : ""}`} readOnly />
            {showIcon && <img src={arrow_down_icon} alt="ARROW DOWN ICON" className="justify-self-end w-3 md:w-5 lg:w-4" />}
          </div>
