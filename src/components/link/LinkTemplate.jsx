...
const LinkTemplate = ({ id, name, main_icon, platform_link, platform_name, handleInput, placeholder, errorMessage, setErrorMessage, removeFunction, link = true, readOnly = true, inputRef, showIcon = false, showOption, setShowOption, availableLinks, changeOption }) => {
  return (
      ...
      <div className="flex flex-col gap-y-2 md:gap-y-4 lg:gap-y-3">
        <main className="relative">
          ...
          <div onClick={() => setShowOption(!showOption)} className={`flex items-center py-1 px-3 md:px-5 gap-x-2 md:gap-x-4 border border-grey-semilight ${!link ? "hover:border-purple-custom hover:shadow-all-sides hover:shadow-purple-light" : ""} transition ease-linear duration-200 rounded-md ${showIcon ? "cursor-pointer" : ""}`}>
            <img src={main_icon} className="w-3 md:w-5 lg:w-4" alt={`${platform_name} ICON`} />
            <input type="text" value={platform_name} onChange={handleInput} className={`text-[8px] md:text-lg text-grey-dark outline-none shadow-inset shadow-grey-light w-full py-2 md:py-3 ${showIcon ? "cursor-pointer" : ""}`} readOnly />
        ...
        <main>
          ...
          <a {...(link && { href: platform_link, target: "_blank" })} onClick={(e) => { !link && e.preventDefault() }} className={`flex items-center py-1 px-3 md:px-5 gap-x-2 md:gap-x-4 border ${errorMessage ? "border-customRed" : "border-grey-semilight"} ${!errorMessage ? "hover:border-purple-custom hover:shadow-all-sides hover:shadow-purple-light" : ""} transition ease-linear duration-200 rounded-md ${link ? "cursor-pointer" : "cursor-default"}`}>
            <img src={link_black_icon} className="w-3 md:w-5 lg:w-4" alt="LINK ICON" />
            <input type="text" name={name} ref={inputRef} placeholder={placeholder} value={platform_link} onChange={handleInput} readOnly={readOnly} className={`text-[8px] md:text-base text-grey-dark outline-none shadow-inset shadow-grey-light w-full py-2 md:py-3 ${link ? "cursor-pointer" : ""}`} />
