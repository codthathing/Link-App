    ...
    <div key={id} className="bg-grey-light rounded-md p-4 md:p-6">
      ...
      <div className="flex flex-col gap-y-2 md:gap-y-4 lg:gap-y-3">
        ...
        <main>
          ...
          <a {...(link && { href: `${(linkValue.startsWith("https://") || linkValue.startsWith("http://")) ? "" : "https://"}${linkValue}`, target: "_blank" })} onClick={(e) => { !link && e.preventDefault() }} className={`flex items-center py-1 px-3 md:px-5 gap-x-2 md:gap-x-4 border ${errorMessage ? "border-customRed" : "border-grey-semilight"} ${!errorMessage ? "hover:border-purple-custom hover:shadow-all-sides hover:shadow-purple-light" : ""} transition ease-linear duration-200 rounded-md ${link ? "cursor-pointer" : "cursor-default"}`}>
            <img src={link_black_icon} className="w-3 md:w-5 lg:w-4" alt="LINK ICON" />
            <input type="text" name={name} ref={inputRef} placeholder={placeholder} value={linkValue} onChange={handleInput} readOnly={readOnly} className={`text-[8px] md:text-base outline-none shadow-inset shadow-grey-light w-full py-2 md:py-3 ${link ? "cursor-pointer" : ""}`} />
